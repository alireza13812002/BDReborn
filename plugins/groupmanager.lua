...
return "ارسال نوشته امجودار در گروه آزاد شد"
 ...
 ---------------Lock gmail-------------------
 local function lock_gmail(msg, data, target) 
 local hash = "gp_lang:"..msg.chat_id_ 
 local lang = redis:get(hash) 
 if not is_mod(msg) then 
 if not lang then 
 ...
 local lock_gmail = data[tostring(target)]["settings"]["lock_gmail"]  
 if lock_gmail == "yes" then 
 if not lang then 
  return "🔒*gmail* _Posting Is Already Locked_🔒" 
 elseif lang then 
  return "🔒جیمیل قفل شده بود🔒" 
 end 
 else 
 data[tostring(target)]["settings"]["lock_gmail"] = "yes" 
 save_data(_config.moderation.data, data)  
 if not lang then 
  return "🔒*gmail* _Posting Has Been Locked_🔒" 
 else 
  return "🔒جیمیل قفل شد🔒" 
 end 
 end 
 ...
 local function unlock_gmail(msg, data, target) 
 local hash = "gp_lang:"..msg.chat_id_ 
 local lang = redis:get(hash) 
  if not is_mod(msg) then 
 if not lang then 
 ...
 local lock_gmail = data[tostring(target)]["settings"]["lock_gmail"] 
  if lock_gmail == "no" then 
 if not lang then 
 return "🔓*gmail* _Posting Is Not Locked_🔓"  
 elseif lang then 
 return "🔓جیمیل بازه🔓" 
 end 
 else  
 data[tostring(target)]["settings"]["lock_gmail"] = "no" save_data(_config.moderation.data, data)  
 if not lang then 
 return "🔓*gmail* _Posting Has Been Unlocked_🔓"  
 else 
 return "🔓جمیل رو باز کردی🔓" 
 end 
 end 
 end
---------------Lock Mention-------------------
...
if data[tostring(target)]["settings"] then		
if not data[tostring(target)]["settings"]["lock_gmail"] then			
data[tostring(target)]["settings"]["lock_gmail"] = "no"		
end
end
...
tdcli_function ({
  ...
  if matches[1] == 'setflood' and is_mod(msg) then
			if tonumber(matches[2]) < 1 or tonumber(matches[2]) > 300 then
				return "_Wrong number, range is_ *[1-300]*"
      end
...
if matches[1] == "online" and is_mod(msg) then
text5 = [[
